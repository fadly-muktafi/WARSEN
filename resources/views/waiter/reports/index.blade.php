                    ...
                    <form action="{{ route('waiter.reports.index') }}" method="GET">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <button type="submit" class="w-full bg-primary-500 hover:bg-primary-600 text-white font-bold py-2 px-4 rounded transition duration-200">
                                    Filter
                                </button>
                                <button type="button" onclick="window.print()" class="w-full bg-secondary-500 hover:bg-secondary-600 text-white font-bold py-2 px-4 rounded transition duration-200">
                                    Print
                                </button>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <p class="text-3xl font-bold text-primary-600">Rp {{ number_format($totalSales, 0, ",", ".") }}</p>
                    </div>
                </div>
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-700">Total Orders</h3>
                        <p class="text-3xl font-bold text-secondary-600">{{ $totalOrders }}</p>
                    </div>
                </div>
            </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($orders as $order)
                                    <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $order->customer->nama_pelanggan ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $order->restaurant_table_name ?? ($order->restaurant_table_id ?? 'N/A') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">Rp {{ number_format($order->total_amount, 0, ",", ".") }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $order->created_at->format('H:i') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-6 text-gray-500">No orders found for this period.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
