                        ...
                        <table class="min-w-full divide-y divide-secondary-200">
                            <thead class="bg-secondary-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-sm font-semibold text-secondary-600 uppercase tracking-wider">Status</th>
                            ...
                            <tbody class="divide-y divide-secondary-200">
                                @foreach ($tables as $table)
                                    <tr class="odd:bg-light even:bg-secondary-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-secondary-800">{{ $table->nomor_meja }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $table->status == 'available' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                {{ ucfirst($table->status) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            <a href="{{ route('admin.tables.edit', $table->id) }}" class="inline-flex items-center px-3 py-1.5 bg-light border border-secondary-300 rounded-md font-semibold text-xs text-secondary-800 tracking-widest shadow-sm hover:bg-secondary-100 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
                                                Edit
                                            </a>
