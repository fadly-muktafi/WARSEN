...
<div class="w-64 h-screen bg-slate-800 text-gray-300 flex flex-col no-print sticky top-0">
    <!-- Logo -->
    ...
    <nav class="flex-1 px-4 py-6 space-y-2">
        ...
        {{-- PERBAIKAN: Menggunakan strtolower() untuk perbandingan yang case-insensitive --}}
        ...
        @elseif (strtolower(Auth::user()->role) == 'waiter')
            ...
            <x-nav-link :href="route('waiter.reports.index')" :active="request()->routeIs('waiter.reports.*')">
        ...
        @elseif (strtolower(Auth::user()->role) == 'kasir')
            <x-nav-link :href="route('kasir.transactions.index')" :active="request()->routeIs('kasir.transactions.*')">
            ...
            <x-nav-link :href="route('kasir.reports.index')" :active="request()->routeIs('kasir.reports.*')">
                <svg class="w-5 h-5 mr-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 1.5m1-1.5l1 1.5m0 0l1 1.5m-2-1.5l-1-1.5m-6.375 5.25h14.25" /></svg>
                {{ __('Reports') }}
            </x-nav-link>
        @elseif (strtolower(Auth::user()->role) == 'owner')
    ...
    <div class="px-4 pb-4">
        <x-dropdown align="top" width="full" content-classes="bg-slate-700 py-1 rounded-md">
            <x-slot name="trigger">
                <button class="w-full flex items-center p-3 rounded-lg text-left text-sm font-medium text-gray-300 hover:bg-slate-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-white transition ease-in-out duration-150">
            ...
            <x-slot name="content">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-dropdown-link 
                        ...
                        class="text-red-400 hover:bg-red-600 hover:text-white focus:bg-red-600 focus:text-white">
                        {{ __('Log Out') }}
                    </x-dropdown-link>
                </form>
            </x-slot>
